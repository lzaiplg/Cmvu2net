...
class ASPPModule(nn.Module):
    def __init__(self, aspp_ratios, in_channels, out_channels, align_corners=True):
        ...
        total_branches = len(aspp_ratios) + 1
        self.concate_projection = nn.Sequential(
            nn.Conv2d(total_branches * out_channels, out_channels, 1, bias=False),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True)
        )
    ...
    def forward(self, x):
        ...
        aspp_outs.append(global_feature)
        
        concate_out = torch.cat(aspp_outs, dim=1)
        
        out = self.concate_projection(concate_out)
        return out
